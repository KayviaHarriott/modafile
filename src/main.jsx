...
function SettingToggle({ title, detail, checked, onChange }) { return <label className="mode-toggle" title={detail}><span><strong>{title}</strong></span><span className="toggle-switch"><input type="checkbox" checked={checked} onChange={(event) => onChange(event.target.checked)} /><span /></span></label> }
