...
function DropPanel({ folder, incomingFile, visible }) {
  ...
  const compress = async () => {
    ...
    try {
      ...
      const result = await invoke('compress_pdf', { inputPath: file.path, folder, targetMb: goal === 'target' ? Number(targetMb) : null })
...
function WindowControls() { const stop = (event) => event.stopPropagation(); return <div className="window-controls" aria-label="Window controls" onMouseDown={stop} onPointerDown={stop}><button className="window-control close" type="button" aria-label="Close KiloFile" onClick={() => getCurrentWindow().close()} /><button className="window-control minimize" type="button" aria-label="Minimize KiloFile" onClick={() => getCurrentWindow().minimize()} /></div> }
function App() {
  ...
  return <main className="canvas"><WindowControls /><div className="content-row" onMouseLeave={collapseOnLeave}><aside className={`pill ${expanded ? 'expanded' : ''}`} onMouseEnter={() => setPillSize(true)} onMouseDown={beginDrag} data-tauri-drag-region aria-label="PDF Squeeze tools"><div className="pill-top">{controls.map((control) => <Control key={control.label} {...control} {...openTool(control.panel)} />)}</div><div className="settings-area"><div className="divider" /><Control label="Settings" icon={settingsIcon} {...openTool('settings')} /></div></aside><DropPanel folder={folder} incomingFile={incomingCompressFile} visible={panel === 'compress'} /><ConvertPanel folder={folder} incomingFile={incomingConvertFile} onFileChange={setConvertLocked} visible={panel === 'convert'} /><SettingsPanel folder={folder} onChoose={chooseFolder} alwaysOnTop={alwaysOnTop} onAlwaysOnTopChange={changeAlwaysOnTop} visible={panel === 'settings'} /></div></main>
}
