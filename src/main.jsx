...
function ConvertPanel({ folder, incomingFile, onFileChange, visible, onComplete }) {
  ...
  const convert = async () => {
    ...
    setProgress(12)
    const progressTimer = window.setInterval(() => setProgress((value) => Math.min(value + 10, 88)), 220)
    try {
...
function App() {
  ...
  const setPillSize = async (next) => { if (panel || navbarMode) return; setExpanded(next); if (bubbleDesign && isTauri()) await invoke('set_bubble_layout', { expanded: next, panelOpen: false }); else await resize(next ? 216 : 128) }
  const showPanel = async (nextPanel) => { setPanel(nextPanel); setExpanded(true); if (bubbleDesign && isTauri()) await invoke('set_bubble_layout', { expanded: true, panelOpen: true }); else await resize(navbarMode ? 560 : 820, 700) }
  const closePanel = async () => { setPanel(null); setExpanded(false); if (bubbleDesign && isTauri()) await invoke('set_bubble_layout', { expanded: false, panelOpen: false }); else await resize(navbarMode ? 560 : 128) }
  ...
  const changeBubbleDesign = async (enabled) => { setBubbleDesign(enabled); localStorage.setItem('modafile-bubble-design', String(enabled)); setPanel(null); setExpanded(false); if (isTauri()) { if (enabled) await invoke('set_bubble_layout', { expanded: false, panelOpen: false }); else await resize(128) } }
  ...
  return <main className={`canvas ${theme} ${bubbleDesign ? 'bubble-design' : ''} ${preferences.reduceMotion ? 'reduce-motion' : ''}`} onMouseEnter={() => { if (alwaysOnTop && isTauri()) invoke('focus_window').catch(console.error) }} onMouseLeave={collapseOnLeave} onPointerLeave={collapseOnLeave}><WindowControls /><div className="content-row" onMouseLeave={collapseOnLeave}><aside className={`pill ${expanded ? 'expanded' : ''}`} onMouseEnter={() => setPillSize(true)} onMouseDown={beginDrag} data-tauri-drag-region aria-label="Modafile tools"><div className="pill-top">{controls.map((control) => <Control key={control.label} {...control} controlRef={(element) => { toolRefs.current[control.panel] = element }} dragActive={dragTarget === control.panel} completed={completionTool === control.panel} {...openTool(control.panel)} />)}</div><div className="settings-area"><div className="divider" /><Control label="Settings" icon={settingsIcon} {...openTool('settings')} /></div></aside><div className="panel-host" onMouseEnter={cancelPanelClose} onMouseLeave={schedulePanelClose} onPointerLeave={schedulePanelClose}><DropPanel folder={folder} incomingFile={incomingCompressFile} visible={panel === 'compress'} autoCompress={preferences.autoCompress} preserveMetadata={preferences.preserveMetadata} onComplete={complete('compress')} /><ConvertPanel folder={folder} incomingFile={incomingConvertFile} onFileChange={setConvertLocked} visible={panel === 'convert'} onComplete={complete('convert')} /><SettingsPanel folder={folder} onChoose={chooseFolder} alwaysOnTop={alwaysOnTop} onAlwaysOnTopChange={changeAlwaysOnTop} bubbleDesign={bubbleDesign} onBubbleDesignChange={changeBubbleDesign} preferences={preferences} onPreferenceChange={changePreference} theme={theme} onThemeToggle={toggleTheme} visible={panel === 'settings'} /></div></div></main>
}
