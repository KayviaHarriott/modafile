...
function DropPanel({ folder, incomingFile, visible, autoCompress, preserveMetadata, onComplete, onActionStart }) {
  ...
  const compress = async () => {
    if (!file) return
    // Compression continues in this mounted panel while the utility returns to its compact pill.
    onActionStart?.()
    setStatus('Compressing…')
    setProgress(12)
    const progressTimer = window.setInterval(() => setProgress((value) => Math.min(value + 11, 88)), 220)
    try {
      if (!isTauri() || !file.path) throw new Error('Please use the Modafile macOS app to compress PDFs.')
...
function ConvertPanel({ folder, incomingFile, onFileChange, visible, onComplete }) {
  ...
  return <section hidden={!visible} className="panel convert-panel" aria-label="Convert files"><h1>Convert files</h1><p>Choose a file and Modafile will show the formats it can make.</p><button className={`drop-zone compact ${file ? 'has-file' : ''}`} type="button" onClick={() => inputRef.current?.click()} onDragOver={(event) => event.preventDefault()} onDrop={(event) => { event.preventDefault(); receive(event.dataTransfer.files[0]) }}><span>{file?.name || 'Drop a file here'}</span><small>{file ? `Available: ${outputs.join(', ')}` : 'HEIC, JPG, PNG, MOV, MP4, M4V'}</small></button><input ref={inputRef} className="file-input" type="file" accept="image/heic,image/heif,image/jpeg,image/png,video/quicktime,video/mp4,.heic,.heif,.mov,.mp4,.m4v" onChange={(event) => receive(event.target.files[0])} />{file && <button type="button" className="clear-file" onClick={() => { setFile(null); setStatus(''); onFileChange(false) }}>Clear file</button>}<div className="conversion-options"><span className="options-label">Convert to</span><div className="format-row">{outputs.map((output) => <button key={output} className={format === output ? 'selected' : ''} type="button" onClick={() => setFormat(output)}>{output}</button>)}</div><div className="supported-note"><span>{isVideo ? 'Video and audio export' : 'Image conversion'}</span><strong>{isVideo ? 'Video format or audio-only file' : 'Choose the format that works best for sharing'}</strong></div></div><button className="convert-action" type="button" disabled={!file || status === 'Converting…'} onClick={convert}>{status === 'Converting…' ? 'Converting…' : `Convert to ${format}`}</button>{status && <span className="compression-status">{status}</span>}</section>
}
...
function App() {
  ...
  useEffect(() => {
    ...
    getCurrentWindow().onDragDropEvent((event) => {
      ...
      if (!path) return
      const suggestedTarget = path.toLowerCase().endsWith('.pdf') ? 'compress' : 'convert'
  ...
  return <main className={`canvas ${theme} ${preferences.reduceMotion ? 'reduce-motion' : ''}`} onMouseEnter={() => { if (alwaysOnTop && isTauri()) invoke('focus_window').catch(console.error) }}><WindowControls /><div className="content-row" onMouseLeave={collapseOnLeave}><aside className={`pill ${expanded ? 'expanded' : ''}`} onMouseEnter={() => setPillSize(true)} onMouseDown={beginDrag} data-tauri-drag-region aria-label="PDF Squeeze tools"><div className="pill-top">{controls.map((control) => <Control key={control.label} {...control} controlRef={(element) => { toolRefs.current[control.panel] = element }} dragActive={dragTarget === control.panel} completed={completionTool === control.panel} {...openTool(control.panel)} />)}</div><div className="settings-area"><div className="divider" /><Control label="Settings" icon={settingsIcon} {...openTool('settings')} /></div></aside><div className="panel-host" onMouseEnter={cancelPanelClose} onMouseLeave={schedulePanelClose}><DropPanel folder={folder} incomingFile={incomingCompressFile} visible={panel === 'compress'} autoCompress={preferences.autoCompress} preserveMetadata={preferences.preserveMetadata} onComplete={complete('compress')} onActionStart={closePanel} /><ConvertPanel folder={folder} incomingFile={incomingConvertFile} onFileChange={setConvertLocked} visible={panel === 'convert'} onComplete={complete('convert')} /><SettingsPanel folder={folder} onChoose={chooseFolder} alwaysOnTop={alwaysOnTop} onAlwaysOnTopChange={changeAlwaysOnTop} preferences={preferences} onPreferenceChange={changePreference} theme={theme} onThemeToggle={toggleTheme} visible={panel === 'settings'} /></div></div></main>
}
