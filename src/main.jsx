...
function Control({ label, icon, onClick, onMouseEnter, completed = false, variant = '' }) { return <button className={`control ${variant}`} type="button" aria-label={label} onMouseDown={(event) => event.stopPropagation()} onPointerDown={(event) => event.stopPropagation()} onMouseEnter={onMouseEnter} onClick={onClick}><span className="icon"><img src={icon} alt="" />{completed && <i className="completion-dot" />}</span><span className="label">{label}</span></button> }
...
function App() {
  ...
  const openTool = (tool) => ({ onMouseEnter: () => showPanel(tool) })
  return <main className={`canvas ${preferences.reduceMotion ? 'reduce-motion' : ''}`} onMouseEnter={() => { if (alwaysOnTop && isTauri()) invoke('focus_window').catch(console.error) }}><WindowControls /><div className="content-row" onMouseLeave={collapseOnLeave}><aside className={`pill ${expanded ? 'expanded' : ''}`} onMouseEnter={() => setPillSize(true)} onMouseDown={beginDrag} data-tauri-drag-region aria-label="PDF Squeeze tools"><div className="pill-top">{controls.map((control) => <Control key={control.label} {...control} completed={completionTool === control.panel} {...openTool(control.panel)} />)}</div><div className="settings-area"><div className="divider" /><Control label="Settings" icon={settingsIcon} {...openTool('settings')} /></div></aside><DropPanel folder={folder} incomingFile={incomingCompressFile} visible={panel === 'compress'} autoCompress={preferences.autoCompress} preserveMetadata={preferences.preserveMetadata} onComplete={complete('compress')} /><ConvertPanel folder={folder} incomingFile={incomingConvertFile} onFileChange={setConvertLocked} visible={panel === 'convert'} onComplete={complete('convert')} /><SettingsPanel folder={folder} onChoose={chooseFolder} alwaysOnTop={alwaysOnTop} onAlwaysOnTopChange={changeAlwaysOnTop} preferences={preferences} onPreferenceChange={changePreference} visible={panel === 'settings'} /></div></main>
}
